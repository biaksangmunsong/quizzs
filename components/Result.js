...
const Result = ({data, setDataToShare, fullName, email}) => {
    ...
    const share = () => {
        if (data){
            const text = `Quizzs Result - ${fullName}`
            const url = `${window.location.origin}/result?email=${email}`
    ...
    return (
        <StyledResult>
            <div className="sub-container">
                <h1>Quiz Result</h1>
                <div className="users-name">{data.fullName}</div>
                <div className={`cards${data.detailedResult.details.length%2 ? " odd" : ""}`}>
